...
class ChakuraBorder(x: Int, y: Int, weaponState: Weapon) : WeaponBorder(x, y, weaponState, price = 2000, unlockdistance = 150) {
    ...
    var weapon: Image = ResController.instance.image(Path.Imgs.Objs.Shuriken.book)
    override var border = ResController.instance.image(Path.Imgs.Objs.border)
    override fun paintComponent(g: Graphics) {
        if (lock) {
            ...
            g.font = font.deriveFont(1, 15.0F)
            g.color = Color.WHITE
            g.drawString("Hold to charge!", painter.left + painter.width * 10 / 50, painter.top + painter.height * 12 / 100)
            g.drawImage(
            ...
            g.color = Color.white
            g.drawString("$price", painter.left + painter.width * 20 / 50, painter.top + painter.height * 45 / 50)
        }
    }
}