...
class MountainFar(x: Int = 0, y: Int = 0): GameObject(x, y, Global.WINDOW_WIDTH, Global.WINDOW_HEIGHT) {
    private val imgBackGround: Image = ResController.instance.image(Path.Imgs.BackGround.mountain)
    ...
    fun paintShift(dx:Double){
        paintShiftCount += dx
        if ((paintShiftCount*544/Global.SCREEN_X).roundToInt() > 272 ){
            paintShiftCount =0.0
        }
    }
